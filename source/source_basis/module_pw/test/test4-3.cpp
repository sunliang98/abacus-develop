...
using namespace std;
TEST_F(PWTEST,test4_3)
{
    cout<<"dividemthd 2, gamma_only: on, xprime: true, check fft"<<endl;
    ...
    //--------------------------------------------------
    lat0 = 4;
    ModuleBase::Matrix3 la(1, 0, 0, 0, 1, 0, 0, 0, 10);
    latvec = la;
    wfcecut = 22;
    gamma_only = true;
    ...
    for(int ig = 0 ; ig < npw ; ++ig)
    {
        ...
        if(pwtest.gdirect[ig].x > 0) 
        {
            rhofg[ig]+=ModuleBase::IMAG_UNIT / (std::abs(pwtest.gdirect[ig].y+1) + 1);
            rhofgr[ig]+=ModuleBase::IMAG_UNIT / (std::abs(pwtest.gdirect[ig].y+1) + 1);
        }
    }    
