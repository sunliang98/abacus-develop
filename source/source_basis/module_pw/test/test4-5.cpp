...
using namespace std;
TEST_F(PWTEST,test4_5)
{
    cout<<"dividemthd 2, gamma_only: on, xprime: true, gamma kpoint, check fft"<<endl;
    ...
    //--------------------------------------------------
    lat0 = 5;
    ModuleBase::Matrix3 la(1, 0.3, 0, 0, 2, 0, 0, 0, 6);
    nks = 1;
    ...
    for(int ik  = 0; ik < nks; ++ik)
    {
                ...
                for(int iy = 0 ; iy < ny ; ++iy)
                {
                    for(int iz = 0 ; iz < nz ; ++iz)
                    {
                        ...
                        if (modulusgk <= ggecut)
                        {
                            ...
                            if(vx > 0) tmp[ix*ny*nz + iy*nz + iz]+=ModuleBase::IMAG_UNIT / (std::abs(v.x+1) + 1);
                            else if(vx < 0) tmp[ix*ny*nz + iy*nz + iz]-=ModuleBase::IMAG_UNIT / (std::abs(-v.x+1) + 1);
                        }
                    }
                }   
            }
        ...
        {
            rhog[ig] = 1.0/(pwtest.getgk2(ik,ig)+1); 
        ...
        for(int ig = 0 ; ig < npwk ; ++ig)
        {
            ...
            EXPECT_NEAR(rhofg[ig].imag(),rhofgr[ig].imag(),1e-6);
#endif
        }
