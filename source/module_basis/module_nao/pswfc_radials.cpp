...
std::string PswfcRadials::read_keyword_value(std::ifstream& ifs, std::string word)
{
    ...
    if (num_quote == 1)
        return steal_from_quotes(ifs, word);
    else
                                                   ...
                                                   const double conv_thr)
{
    ...
    for (auto it = lzeta_rvalues.begin(); it != lzeta_rvalues.end(); it++)
    {
        ...
        smooth(rgrid,
               rvalue_padded,
               0.1); // smooth the radial function to avoid high frequency noise in FFT-spherical bessel transform
                                  ...
                                  const int rank)
{
    ...
    if (rank == 0)
    {
        ...
        std::string line = "";
        // read element
        while (!startswith(line, "element=") && !ifs.eof())
            ifs >> line;
        symbol_ = read_keyword_value(ifs, line);
        // read lmax
        while (!startswith(line, "l_max=") && !ifs.eof())
            ifs >> line;
        lmax_ = std::stoi(read_keyword_value(ifs, line));
        // read ngrid
        while (!startswith(line, "mesh_size=") && !ifs.eof())
            ifs >> line;
        ngrid = std::stoi(read_keyword_value(ifs, line));
        // read nzeta
        while (!startswith(line, "number_of_wfc=") && !ifs.eof())
            ifs >> line;
        ...
        for (int il = 0; il < lmax_ + 1; il++)
            nzeta_[il] = 0;
        for (int iz = 0; iz < nzeta; iz++) // read chi-by-chi
        {
            // find the next <PP_CHI.> tag
            while (!startswith(line, "<PP_CHI.") && !ifs.eof())
                ifs >> line;
            // read l
            while (!startswith(line, "l=") && !ifs.eof())
                ifs >> line;
    ...
    for (int i = 0; i < nchi_; i++)
        Parallel_Common::bcast_double(rvalues[i].data(), ngrid);
